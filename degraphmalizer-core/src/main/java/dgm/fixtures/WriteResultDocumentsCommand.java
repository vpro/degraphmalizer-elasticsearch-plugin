...
public class WriteResultDocumentsCommand implements Command<List<ID>> {
    ...
    @Inject
    public WriteResultDocumentsCommand(Client client, Provider<Configuration> cfgProvider, Provider<FixtureConfiguration> fixtureConfigurationProvider) {
        this.client = client;
    ...
    private List<ID> writeDocuments(String index, String type) {
        List<ID> ids = new ArrayList<ID>();
        try {
            File resultsDirectory = fixtureConfigurationProvider.get().getResultsDirectory();
